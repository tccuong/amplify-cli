...
/**
 * 
 * if secrets is defined , Function stores the OAuth secret into parameter store with Key /amplify/{appid}/{env}/{authResourceName}_HostedUIProviderCreds
...
export const syncOAuthSecretsToCloud = async(context: $TSContext, authResourceName: string, secrets?: $TSObject): Promise<void> => {
    ...
    const hostedUI = authCliInputs.cognitoConfig.hostedUI;;
    if(!_.isEmpty(authProviders) && hostedUI)
    {
        if (!_.isEmpty(secrets)) {
            ...
            // if not then fetch the secerts from cognito and insert in paraeter store
