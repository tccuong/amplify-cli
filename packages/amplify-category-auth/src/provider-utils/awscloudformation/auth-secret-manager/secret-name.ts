  ...
  */
 export const getFullyQualifiedSecretName = (secretName: string, resourceName: string, envName?: string) =>
   `${getOAuthSecretPrefix(resourceName, envName)}${secretName}`;
  ...
  * If envName is not specified, the current env is assumed
  */
 export const getOAuthSecretPrefix = (resourceName: string, envName?: string) =>
   path.posix.join(getEnvSecretPrefix(envName), `AMPLIFY_${resourceName}_`);
