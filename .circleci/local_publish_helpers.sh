...
function uploadPkgCli {
    ...
    aws s3 cp amplify-pkg-win.exe s3://pkg-cli-ci-do-not-delete/$(echo $version)/amplify-pkg-win-$(echo $hash).exe
    aws s3 cp amplify-pkg-macos s3://pkg-cli-ci-do-not-delete/$(echo $version)/amplify-pkg-macos-$(echo $hash)
    aws s3 cp amplify-pkg-linux s3://pkg-cli-ci-do-not-delete/$(echo $version)/amplify-pkg-linux-$(echo $hash)
    if [ -z "$NPM_TAG" ]; then
        exit 0
    fi
