...
function uploadPkgCli {
    ...
    if [[ "$CIRCLE_BRANCH" == "release" ]] || [[ "$CIRCLE_BRANCH" == "beta" ]] || [[ "$CIRCLE_BRANCH" =~ ^tagged-release ]]; then
        ...
        aws --profile=s3-uploader kms sign --key-id alias/s3-sign-verify --message fileb://message.sha512 --message-type DIGEST --signing-algorithm RSASSA_PSS_SHA_512 | jq -r .Signature | base64 --decode > signature-digest.sign
